...
from version_control import version_protocol
    ...
    def start(self,debug = True):
        ...
        try:
            while True:
                ...
                clientIP, clientPORT, io_payload = version_protocol.decode_DataPacket(data)
                
                if debug:
                        Debug.log("data",data)
                        Debug.log("addr",addr)
                        Debug.log("clientIP",clientIP)
                        Debug.log("clientPort",clientPORT)
                        Debug.log("io_payload",io_payload)

                addr = (clientIP, clientPORT)
                response = self.handle_packet(io_payload, addr)

                if response:
                    if debug:
                        Debug.log(type(response), "response_type")
